...
| 项 | 值 | 来源与理由 |
|---|---|---|
| 跑道 | 3 条，x = -2.2 / 0 / 2.2，玩家 z=0 | 地铁跑酷经典三车道制；间距 2.2 使三道同屏可读（相机视锥内），换道决策一瞥完成 |
| 相机 | (0, 4.1, 7.8)，lookAt (0, 1.35, -10)，x 跟随玩家 0.55 倍 | 汪汪跑酷模板实证视角：角色与前方障碍决策区同屏；0.55 跟随 = 换道视野提前量与画面稳定的折中 |
| 速度 | 12 起，+0.45/s，上限 30；受击 0.45× 减速 0.5s | 无尽加速经典节奏；上限与刷怪行联动——30 时排间距 max(16, 34.5) 仍保 ≥1.1s 反应窗；机器人 4×120s 零受击实证上限可玩 |
| 跳跃 | vy=8.5，g=25 → 高 1.45、滞空 0.68s | 算术：高 = vy²/2g = 1.445、滞空 = 2vy/g = 0.68s；与障碍行构成判定链（见表下） |
| 滑铲 | 0.65s，判定高 1.0；空中按滑铲 vy=-12 快速下砸 | 判定高 1.0 < 横梁底 1.55 → 滑可过梁；0.65s 覆盖单障碍通过时长；空中快砸承原版技巧 |
| 换道 | lane 索引 + x 插值 dt×12，带侧倾 | 换道 ≈0.2s（插值速率 12），远小于最短反应窗 ≥1.15s——换道本身永远不构成死局 |
| 障碍 | 护栏（高 1.0，跳）、横梁（底 1.55，滑）、火车（满高长 10-15，换道） | **判定链：每类障碍恰有一种解法**——护栏 1.0 < 跳高 1.45 可跳；横梁底 1.55 > 跳高 1.45 跳不过、且 > 滑铲高 1.0 必滑；火车满高不可跳滑、长 10-15 超单跳滞空×速度 → 必换道 |
| 刷怪 | 每排占 1-2 条道、**永不占满 3 条**；排间距 max(16, speed×1.15)（≥1 秒反应窗） | "永不占满" = 任意局面恒有合法解（公平性铁律）；排间距 ≥ speed×1.15 → 反应窗 ≥1.15s > 人类反应下限 ~0.8s |
| 收集物 | 5 连排：直线 y=0.8 或弧线 y=1.6-\|i-2\|×0.45 | 弧线顶点 1.6 贴跳高 1.45 上沿——跳跃轨迹即收集路径暗示（承平台跳跃"收集物弧线"惯例） |
| 生命 | 3，受伤无敌 1.5s（visible 闪烁） | 汪汪跑酷模板实证；无敌时长 = 脱离危险区下限 |

**改数纪律**：动上表任何数值前，先重推"障碍-动作判定链"（护栏 < 跳高 < 横梁底、滑铲高 < 横梁底、火车不可跳滑 → 换道），再跑 `runner_bot` 4×120s 实证——链条任一环节断裂都会产生无解障碍。
...
- 结构 = 底色 + 大块明暗色斑（破单调的关键）+ 中景细节（碎石/草簇）+ 高光点缀

**轨道几何化**（"铁轨丑"的根因是全画在贴图里没有厚度）：
- **钢轨**：6 根长条盒（0.09 见方 × 400），材质数组顶面亮钢 #e8edf0 / 侧面暗钢 #7d868d，凸出枕木 0.09 受光落影——贴图里只留"落位暗条"，画假钢轨会和几何体重影
- **枕木**：InstancedMesh 720 根（3 道 × 240，单绘制调用），数据数组存 z，每帧 `s.z += spd×dt` 到界 -400 回收，`setColorAt` 做微色差——比 240 个独立 Mesh 省 200+ 绘制调用
- 分工：**贴图只画"静止的"（道砟底/色斑/落位影），几何做"有厚度的"（钢轨/枕木）**
...
回收时按"带"重置 x（scenItems 存 `kind` 标记），否则近轨带会被随机到远处。

**主题配方：城市铁路走廊（地铁跑酷风）**——用户说"不像地铁跑酷"时按这个改：
- **地面定主题**：大草坪=乡村，砟石/水泥=站场。地铁跑酷全图没有草坪，地面全是灰暖砟石（#a9a294）+砖缝杂草+纸屑
- **两侧压迫感**：围墙（3.2m 混凝土板+立柱+涂鸦，静态长墙）+ 楼群峡谷（贴墙后排高 5~13 的窗格楼，x≥12.5 防穿墙）——"走廊感"比"远景好看"重要
- **铁路标志物**：电线龙门架（横跨三轨，24m 一座，过顶即是速度感）+ 信号机 + 近轨货箱/杂草/碎石
- **树只是点缀**：高过墙头（4m+）才露得出，10 棵足够
- **治"地面悬空"**：路基边坡（轨道两侧斜面长条）+ 轨道贴图两缘压暗落影——轨道必须"铺"在坡上，不能浮在平面上
...
**主线就是 low poly 复刻**：体素方块（全 Box 拼装 + 像素凸眼）与描边卡通世界同源，零外部依赖、零许可问题；实测教训：AI 图生 3D 的"软陶写实风"放进卡通低模世界会打架（用户原话"跟一坨粑粑一样"），不要为"更真实"牺牲风格统一。
...
8. **静态带纹理长走廊 = 贴着镜头往前飞**：围墙/站场地面这类 400m 长物如果用静态网格+不滚动纹理，轨道在后退它们却钉在原地，视觉上就是"背景跟着镜头往前"。必须滚纹理（`offset += spd×dt/单元长`，与轨道同速）；云/天幕则完全静止（云漂移速度 0.15 都会被读成"跟着跑"）
9. **盒体 ±x 面 UV 互为镜像**：左右对称长墙共用一张滚动纹理，同向滚必有一侧反向（涂鸦往远处跑=跟着镜头飞）——克隆一份纹理反向滚。**A/B 面滚动方向验证要两面都看**（上次只验了左墙就上线，被用户抓回）
