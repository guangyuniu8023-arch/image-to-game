...
本文件是**素材生产与验收策略，不是素材设计的替代品**。调用本文件前，GDD 必须已按 [new-type.md](new-type.md) 闭合角色职责、主题/视觉简报、素材系统和 Sprite 状态：每项素材为什么存在、服务哪条玩法/信息、有哪些状态、游戏内尺寸与锚点、用 AI/代码/混合哪种方式、失败时怎样回退。

- **输入**：GDD 中已通过玩法—视觉一致性检查的视觉简报、素材生产清单、Sprite/动画状态计划。
- **本文件处理**：把设计决策转成 prompt 或 Canvas 配方，执行抠图、生图、清理、切帧、压缩、回退注入与 contact sheet 验收。
- **输出**：可直接集成的素材文件、Sprite 帧、prompt 对账记录和一致性验收证据。

若生成过程中发现缺少状态、尺寸与碰撞冲突、HUD 遮挡或素材没有玩法职责，必须退回 GDD 修正；禁止在生产阶段静默新增设计。
...
4. **一致性验收**：全套素材与角色图**并排拼一张 contact sheet** 目检——描边粗细、上色方式、明暗方向、配色家族一致，像"这个角色世界里的东西"；单件跑偏只重新生成那一件。**拼图工具**：`python3 scripts/make_contact_sheet.py 主角.png 素材目录/ -o contact_sheet.png`（棋盘格衬底透明白边立现，每件带文件名标注）。 目检过后再过 verification.md 第 3 级**自评清单**（诚实反思 6 条：存在感/呼吸vs空旷/违和感/构图避让/职责可读/风格打架），不合格先调再交付。
...
**融合 = 形态转换，不是贴图**（用户过审的打砖块实证）：按玩法视角把角色改造成"可操控素材"——贴底背面半身（接球/发球类：腰出画大图贴屏幕底、背面举手、弹起面对齐手部，IP 靠发色/配饰辨认不靠正脸）、正面全身（跑跳类）、Q 版（扁平世界需要时）。形态由玩法定；原图只提供 IP 特征（发色/标志元素/配色），**禁止"原封立绘贴进场景当摆设"**（实踩：118px 正面小立绘贴挡板，又孤独又读不懂，用户驳回；换背面半身贴底接球手后过审）。
...
**本节输入是 GDD 的 Sprite/动画状态计划**；这里只规定如何稳定产出已决定的序列帧，不负责替游戏选择动画状态。先确认角色是 A 操作主体、B 主体驱动者还是 C 看板娘，再按状态计划逐项生产；不需要 move 状态的类型不得为了套流水线强造跑步帧。

**A 操作主体若在 2D 中左右奔跑，move 状态必须有序列帧（4 帧）**；跳跃/站立/受伤等状态可按 GDD 选静态图、序列帧或程序化反馈。B/C 档按事件状态计划执行，不套跑步规则。2 帧奔跑会明显顿挫，4 帧才流畅。
