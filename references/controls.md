...
本策略分两层：**语义输入 → 输入模式**。操控方案按**输入维度**推导，不按游戏类型查表——所有项目在 [gdd-strategy.md](gdd-strategy.md) 模块 1 声明输入维度即可继承现成方案。
...
维度组合时各轴独立落模式（如“移动+瞄准”= A + 右半屏虚拟摇杆）。三个模式覆盖不了的新维度，先回项目 GDD 模块 1 定义清楚，再为该维度设计可复用方案。

## 模式 A：虚拟键区实现规范

按键 ≥88px 放拇指热区（屏幕下 1/3），动作键大于方向键（如 116px vs 96px），半透明（opacity .45、按下 .7），容器 `touch-action:none` 防手势冲突（**数值来源**：88px = 触控目标下限惯例——iOS HIG 44pt / Android Material 48dp——在 540 宽舞台上的换算余量；116/96 配比与透明度档承已验证的平台跳跃模板；注意半透明指整键透明度，字形仍须深色实色+描边，白字+浅底会“隐形”，见 ui-kit.md 陷阱 2）；**多点触控**按 `touch.identifier` 跟踪，支持“按住方向同时跳”；按键映射到与键盘同一语义变量，**松开触发与 keyup 完全相同的逻辑**（平台跳跃可变跳高实例见 platformer-patterns.md）。控件常驻，桌面浏览器直接可点。
