...
5. **对齐必须放大验（v5 驳回案例）**：整图缩略目检"看着居中"**不算数**——HUD chip/按钮/标题栏必须逐个裁原生分辨率 region 放大 2~3 倍检查（文字在底材内的上下留空差 ≤2px、icon 与文字中线共线）。v5 实测：整图目检报"居中"的 HUD，放大后数字偏上 11px 被用户抓包——根因是测量前没钉 textBaseline（见④钉基线铁律）。截图里亮度剖面测行范围比阈值法可靠：胶囊亮区行、文字墨区行分开测再比中心。
