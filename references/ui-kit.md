...
游戏的标题页、HUD 和结算卡先在项目 GDD 中完成信息与视觉设计，再进入生产。质感和主题装饰优先用 AI 底材，严格几何、动态数值和轻量组件用代码，常见方案是二者混合；**文字、数字、进度条和状态变化始终由代码绘制**，避免 AI 写字和多版本漂移。
...
- **阶段 B：UI 生产、集成与验收**。只有项目 `GDD.md` 已按 [gdd-strategy.md](gdd-strategy.md) 通过一致性门后才能进入；用③④⑤把已批准决策转成底材、代码组件、状态、回退和截图证据。
