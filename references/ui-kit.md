...
本文件是**HUD/UI 生产、集成与验收策略，不替代 HUD 设计决策**。调用前，GDD 必须先回答：玩家做当前决策必须知道什么、信息优先级是什么、放在哪个屏幕区域、需要哪些组件及状态、角色画风与游戏节奏怎样影响 UI 的线条/质感/形状/密度。

- **输入**：GDD 中已确定的 HUD 信息架构、布局方向、组件状态清单和 UI 四属性。
- **本文件处理**：把这些决策转成底材 prompt、九宫格/代码叠字、状态表现、回退方案和对齐验收。
- **输出**：UI 底材、主题 icon、可运行组件、标题/HUD/结算集成结果与截图证据。

若执行时发现 HUD 缺少关键决策信息、信息过载或遮挡玩法，退回 GDD 修正；不得靠临场增加 chip、按钮或文案补洞。
...
复用 GDD 已记录的 assets.md 角色图特征与主题域（不重新看图），加游戏类型，复核并执行 UI 四属性：
