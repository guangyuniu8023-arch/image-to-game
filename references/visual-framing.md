...
第一个 case 必须是 `name: "boot"`、`entry: "natural"`：审计器在页面自然启动后直接调用只读 `snapshot()`，不能先让项目代码切状态。其余 case 使用 `entry: "scripted"`。这样“代码里画了标题页，但启动流程马上跳过标题”的不可达封面不能靠强制状态截图混过验收。
...
`snapshot` 只能读取自然启动后的现状；审计器在任何 `runCase` 之前调用它并截图。`runCase` 必须调用真实状态机、相机更新和投影函数；禁止直接给 `state`、角色落点、相机 target 或成功事件赋预期值后返回。`transition` case 另返回触发紧前的 `result.before` probe、实际触发的 `events` 和仅包含触发后 probe 的 `samples`，且 `events` 必须包含合同声明的 `trigger_event`。每个推进循环有固定 `dt`、最大帧数和有限值断言。
