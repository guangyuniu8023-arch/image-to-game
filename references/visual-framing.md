...
每个 `transition` case 必须返回触发前的真实 probe，并声明 `before_state`、`min_target_delta_px`、`min_camera_delta_px`、`required_target_axes` 和 `min_axis_target_delta_px`。阈值来自本次世界位移、投影和构图需求。审计不仅检查“收敛到自己声明的 target”，还检查目标确实改变、镜头确实移动，以及需求决定的屏幕轴确实产生位移，防止把常量 target 包装成成功跟随。
...
`snapshot` 只能读取自然启动后的现状；审计器在任何 `runCase` 之前调用它并截图。`runCase` 必须调用真实状态机、相机更新和投影函数；禁止直接给 `state`、角色落点、相机 target 或成功事件赋预期值后返回。`transition` case 另返回触发前 `result.before` probe 和实际触发的 `events`，且必须包含合同声明的 `trigger_event`。每个推进循环有固定 `dt`、最大帧数和有限值断言。
