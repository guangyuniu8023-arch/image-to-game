...
第一个 case 必须是 `name: "boot"`、`entry: "natural"`：审计器等待 `window.__game.ready === true` 后直接调用只读 `snapshot()`，不能先让项目代码切状态。它只证明资源就绪后的自然状态可被忠实采样；启动状态是玩法、标题页还是其它明确需求，由项目 GDD 决定，审计器不按 `state` 名称推断产品策略。其余 case 使用 `entry: "scripted"`。默认无封面、加载后直接开玩的策略定义在 Skill 与 GDD；用户显式要求的启动页仍然是合法项目。
...
正式构图先生成少量运行时候选：紧凑、平衡、决策空间优先。候选必须由同一个游戏渲染器、投影、HUD 和 Sprite 可见像素框产生。由 Agent 根据角色可读性、目标判断、路径清楚度、遮挡和空旷程度选出最强方案；简单 H5 一次比较即可，不循环抽图。脚本检查用于暴露问题，不是继续生产或交付候选的许可证。

固定数值只用于安全和稳定断言，例如对象不能被裁掉、镜头锁定不能漂移、轨迹必须在帧上限内收敛。审美尺度以批准的 Seed 运行时预览为基线，正式版本与该基线比较，不与全局万能比例比较。

## 4. 建立真实构图骨架并接入 Seed

GDD 完成后、完整素材生产前，先建立最小运行时构图骨架：
...
随后生成游戏内 Seed Frame，把它接入该骨架，使用真实 Sprite 可见像素框重新计算取景。若 GDD 选择了必要动画，可继续按 Seed → 整条 → 归一化流程完成首个游戏版本；简单 H5 不强制停在审批页。用户明确批准过的 Seed 后续不得悄悄替换。
...
```json
{
  ...
  "cases": [
    ...
    },
    {
      ...
      "transition_rationale": "保留落点后的空间连续性，并在下次蓄力前收敛",
      "min_transition_samples": 3,
      "min_target_delta_px": 16,
      "min_camera_delta_px": 16,
      "required_target_axes": ["y"],
      "min_axis_target_delta_px": 12,
      "required_visible": ["player", "nextTarget"],
      "required_render_sources": {
        "player": {
          "seed": ["generated-seed"],
          "production": ["generated-sprite"]
        }
      },
      "max_frames": 30
    }
...
`required_render_sources` 也必须按状态和阶段推导。承担正式角色职责的主实体不能接受 `fallback`；Seed 阶段通常是 `generated-seed`，白名单机制动作在 Production 对应状态应收紧为 `generated-sprite`。probe 必须报告当前真实 `render_source`，资源加载失败回退可以保证游戏不白屏，但不能冒充正式角色验收通过。

每个 `transition` case 必须返回触发前的真实 probe，并声明 `before_state`、`min_target_delta_px`、`min_camera_delta_px`、`required_target_axes` 和 `min_axis_target_delta_px`。还必须根据本次玩家感知、空间连续性与下一决策时机，写入 `transition_mode: "smooth" | "cut"` 及 `transition_rationale`；这是需求推导的运行断言，不是用户表单或游戏类型路由。`smooth` 还必须声明不小于 3 的 `min_transition_samples`：触发后首帧不得已经 settled，target 在跟随期间保持稳定，中间帧真实收敛。`cut` 则必须在首个触发后 probe 已稳定；不能用硬切伪装 GDD 声明的平滑跟随。`result.before` 必须在会改变相机 target 的 `trigger_event` **紧前**采集；返回的 `samples[0]` 必须是该事件生效后的第一个 probe，不得混入事件前帧。为进入可复现场景而做的快进可以在内部执行，但不能把快进样本放入返回轨迹。事件后样本必须趋近新 target，最后一帧的真实状态必须已经稳定；不得为了通过审计将未收敛的 probe 重命名为 `cameraSettled`。阈值来自本次世界位移、投影和构图需求。审计不仅检查“收敛到自己声明的 target”，还检查目标确实改变、镜头确实移动，以及需求决定的屏幕轴确实产生位移，防止把常量 target 包装成成功跟随。

`artifacts` 是**当前审计 phase 真实存在并参与运行时哈希**的文件清单，不是未来生产计划。以 `seed` 标签审计时不得把尚未生成的动作 raw、strip、frames 或其它未来文件写入其中；后续动作产物由 Sprite/角色生产记录与 `production` 标签诊断。这里的 phase 只是报告标签，不控制工具调用或候选交付。目标视口必须与运行时 probe 返回的 `sample.viewport` 完全一致。
...
window.__game.visualAudit = {
  snapshot() {
    // 只读返回页面自然启动后的当前 probe；不得 init、切状态或生成测试世界。
    return { samples: [/* current probe */] };
  },
  runCase(name) {
...
字段名是运行时协议，不得缩写：相机必须返回 `x/y/zoom/targetX/targetY/targetZoom`；内部可以使用其它变量名，但 probe 需要映射到这些标准键。状态机、测试入口和资源成功或明确回退后，将 `window.__game.ready` 设为严格布尔 `true`，不能省略或用 truthy 对象代替。

`snapshot` 只能读取资源门放行后的自然现状；审计器在任何 `runCase` 之前调用它并截图。它不得初始化本局、点击隐藏入口或强制切换项目状态。当前自然状态应是可玩界面还是用户明确要求的启动界面，只由 GDD 决定，不由审计器改写。`runCase` 必须调用真实状态机、相机更新和投影函数；禁止直接给 `state`、角色落点、相机 target 或成功事件赋预期值后返回。`transition` case 另返回触发紧前的 `result.before` probe、实际触发的 `events` 和仅包含触发后 probe 的 `samples`，且 `events` 必须包含合同声明的 `trigger_event`。每个推进循环有固定 `dt`、最大帧数和有限值断言。
...
脚本直接启动浏览器、运行合同 cases、保存逐 case 截图和实际轨迹，并生成 `evidence/visual-baseline.json`。非零退出码表示构图仍有问题，应加入候选报告；它不阻止素材生成，也不隐藏已存在的 HTML。
...
动态检查覆盖必须可见对象、HUD 遮挡、有限坐标、锁定状态漂移、过渡是否趋近目标和帧上限内收敛。若本项目确实产生了经用户选定的 Seed 构图基线，再显式加 `--baseline <项目目录>/evidence/visual-baseline.json` 比较正式版本的主体/对象组尺度漂移；未指定 baseline 时不隐式增加 Seed 预审节点。一旦显式传入 baseline，文件缺失、合同哈希不匹配或已批准素材变化都必须报错。把 case 截图用 `make_contact_sheet.py` 合并，一次性交给 VLM 或人工检查角色存在感、路径清晰度、空旷和移动是否突兀；失败项与候选一起交付。
...
每次只修改所属子系统并重跑同一 case。简单 H5 默认一次视觉诊断；只有报告给出明确单一根因时允许一次定向修复和一次复测。浏览器在 ready 前抛出的首批 JavaScript 异常必须随失败报告透传，修复实际异常，不猜端口或等待时长。复测仍失败时立即停止自动修改，返回当前 HTML、具体 case、轨迹和截图；禁止删除/改名失败报告来假装通过。
